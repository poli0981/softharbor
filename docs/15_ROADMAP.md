...
| 2026-07-29 | **D23** third `logo` form `monogram` — renders the app's initials when no brand mark is available. 191 of 290 entries need it: Simple Icons carries only ~95 of the catalogue, and docs/14 §3d sanctions no other source than the vendor's own assets. `local:` stays for vendor-official SVGs; the `webp` alternative was dropped as declared-but-unimplemented (the render path inlines SVG) | ✅ |
| 2026-07-29 | **D24** the seed batch is **exempt from docs/03 §1's one-app-per-PR rule** — 286 entries landed in one `data:` commit. Per-app PRs would have meant 286 reviews of machine-uniform files, and the L3 link check is the only gate that genuinely wants per-app granularity. The rule resumes for every entry added after the seed | ⚠ review |
| 2026-07-29 | `/apps` measured at **96.5 KB gzip** (VI 100.7 KB) with 290 apps — under the 200 KB budget in §2 item 9, so pagination stays deferred even though the 250-app count trigger has passed | ⚠ review |
| 2026-07-29 | **`communication` filled** — 37 entries, the last empty category. Dataset now **326 apps / 697 pages**; `/apps` re-measured at **112.2 KB gzip** (VI 116.9 KB), still inside the 200 KB budget. Extrapolating ~16 KB gzip per 37 apps, pagination becomes due somewhere past **~600 apps** | ⚠ review |
| 2026-07-30 | **docs/09 §6 vetting run across all entries.** 320 → `clean`, each with its VirusTotal permalink in `security.evidence`; **6 held at `unverified`** — `peazip` (Criminal IP phishing + Xcitium malware), `tor-browser` (Webroot + alphaMountain), `protonvpn`, `rufus`, `synfig-studio`, `windscribe` (one vendor each). All six look like the false positives these tool categories routinely attract, but §6.6 says anything uneasy stays `unverified`, and `unverified` is still listable — so nothing is lost by not claiming `clean`. Their evidence URLs are recorded too, so a reader can see exactly what was found | ✅ |
| 2026-07-30 | **`workbox.navigateFallback: ''`** — omitting the key does not disable it. vite-plugin-pwa's `defaultWorkbox` sets `navigateFallback: 'index.html'`, so the SW registered `NavigationRoute(createHandlerBoundToURL('/'))` despite the config comment claiming otherwise. `/` was never precached, so every SW-handled navigation threw `non-precached-url :: [{"url":"/"}]` — visible on returning from an external link. Had `/` been precached it would have been worse and silent: that route answers **every** navigation with its bound URL, i.e. the home page for the whole site. New gate **`pnpm check:sw`** asserts the emitted `dist/sw.js` instead of trusting the config, covering all four SW bugs to date | ✅ |
| 2026-07-30 | **Repo docs added**: `README.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md` (Contributor Covenant 2.1 by reference, per H-decision). CONTRIBUTING carries the auto-close list — malicious code (confirmed ⇒ permanent ban), invisible/confusable characters, unofficial links, affiliate params, unevidenced `clean` status, padded or unedited-AI prose, gate-weakening, and GitHub AUP violations | ✅ Kokone |
| 2026-07-30 | **`build.inlineStylesheets: 'never'` — a CSP decision, not a perf one.** A meta CSP is applied when the document *parses*, and a ClientRouter swap does not reparse, so the policy in force stays the one from the page the visitor **started on**. Any page-specific inline style is therefore refused on arrival from a page that lacked it. `/apps` alone carried ShAppCard's scoped CSS, so soft-navigating there from a detail page silently dropped the stretched-link `::after` — the whole-card click target — and unhid `.sr-only` text. External stylesheets need no hash under `style-src 'self'`, so every page accepts them; they also cache once instead of shipping per page. `check:styles` now fails on any style only *some* pages allow | ✅ |
| 2026-07-30 | **Verify CSP against `astro preview`, never `astro dev`.** The dev server does not emit the per-page CSP meta, so a violation check there returns a false clean — which is exactly what happened, and the bug above was only caught by testing production | ✅ |
| 2026-07-30 | **`check:styles` extended to `<style>` ELEMENTS**, not just `style=` attributes. It now hashes every emitted style block and fails if the page's own CSP does not allow it. The attribute sweep had been the only check, which is why 1.5 MB of refused CSS shipped green; the extension was verified by reintroducing `transition:name` and watching it flag all 652 pages. Third silent-CSP failure this project has had — the lesson each time is that a policy is only enforced if something compares the output against it | ✅ |
| 2026-07-30 | **`transition:name` removed** (docs/06 §7 withdrawn). Astro emits a per-slug `<style>` for it and omits that style's hash from the page's own CSP, so it was refused on every hard load *and* every swap — the last console error. Allow-listing all 326 hashes costs ~17 KB per page head (~12 MB across the build) for a 180 ms effect; a nonce needs a Worker (hard rule 1). Nothing was lost: a morph needs the name on both sides and `ShAppCard` never had one, so it had never animated. Removing it also drops **~1.5 MB of CSS the browser was already refusing**. The cross-page fade is unaffected — that style is hashed correctly | ✅ Kokone |
| 2026-07-30 | **Sorting was broken in production and no gate could see it.** `grid.ts` reordered with `el.style.order`, which CSP `style-src` refuses (D20) — so every navigation logged a CSP violation and the grid never reordered. `check:styles` scans built HTML and this assignment only ever existed at runtime, so it passed. Fixed by moving the DOM nodes instead; the comment claiming CSS order was needed to protect focus and an entrance animation was wrong on both counts — there is no entrance animation, and every control lives outside the grid. **`src/lib/grid.test.ts` now covers it**, and was checked to fail against the old implementation | ✅ |
| 2026-07-30 | **`prefetch: false`.** `<ClientRouter />` prefetches on hover, and Cloudflare answers speculative requests with a bare 503 plus `Cf-Speculation-Refused: prefetch refused: disabled for worker requests`. Reproducible with a single `Sec-Purpose: prefetch` header. Every hover cost a failed request and a console error for zero benefit; real navigation was never affected | ✅ |
| 2026-07-30 | **New legal document: AI Disclosure** (docs/14 §3f), naming the model, version and developer. `LEGAL_VERSION` bumped 07-28 → 07-30: the accepted set (Disclaimer/Terms/Privacy) did not change, but that constant also stamps "last updated" on every legal page and would otherwise have dated a new document before it existed. Re-opening the gate pre-launch costs nothing | ✅ Kokone |
| 2026-07-30 | Legal pages lost their in-body document nav — `ShFooter` lists all six on every page already (docs/06 §5) | ✅ Kokone |
| 2026-07-30 | **L3 caught ten dead official links** the human passes had missed — the first time the link check ran against the whole catalogue. Worst of them: **`henrypp.org/product/simplewall` now 301s to `slync.io`**, an unrelated commercial site, so the vendor domain has lapsed or changed hands; `exifcleaner.app` no longer resolves; `giuspen.net` fails TLS connect. All three now point at the project's own repo, the only live official home. The rest were plain 404s on moved download pages | ✅ |
| 2026-07-30 | **`ds4windows` dropped.** `Ryochan7/DS4Windows` is gone (404), upstream `Jays2Kings` archived since 2023, and the newest fork `schmaldeo` is archived too — no maintained official home, which C4/C5 require. Deleting rather than quarantining is correct: docs/03 §5 allows deletion for an entry that never shipped, and quarantine is for `flagged`, not for abandonment. Catalogue 327 → **326** | ✅ Kokone |
| 2026-07-30 | Link check now accepts **403/429**. 14 of the 26 CI failures were bot mitigation on plainly-live sites (winehq, dolphin-emu, rpcs3, gitlab, MSI, Nexus, Epic, Enpass). A checker wrong about a seventh of the catalogue gets ignored, which costs more than the 403 it would eventually be right about. 404/500/connection failures still break the build. **Spoofing a browser `--user-agent` was tried first and is explicitly rejected**: it cleared the 403s but made gitlab.gnome.org answer 406 and whatsapp.com 400, both of which pass on lychee's default agent — no single UA suits every vendor, so it trades one false-positive set for another | ✅ |
| 2026-07-30 | `security.evidence` was collected by the schema and demanded by docs/09 §6 but **rendered nowhere** — it had always been `null`, so the gap was invisible until 327 permalinks landed. The detail page's security line now links it (docs/06 §5, `detail.evidence`) | ✅ |
| 2026-07-30 | **C3 rulings approved by H7** — the freemium entries below are confirmed acceptable as `free` | ✅ |
| 2026-07-29 | **C3 rulings pending for the freemium communication entries** — Slack (90-day history), Zoom (40-min meetings), Microsoft Teams (60-min calls), Discord, Beeper, and eM Client (`free-onetime`, one device / two accounts) all enter as `free` on the grounds that the free tier is self-sufficient for desktop use. Each summary states its limit rather than hiding it. Hard rule 3 is satisfied — none is subscription-*only* — but docs/04 §3 wants the rulings logged, and these are H7's to confirm | ⚠ Kokone |
| 2026-07-29 | Logo remaps refuse any Simple Icons name that is also a common word (`session`, `wire`, `mumble`, `element`, `line`, `signal`, `zoom`, and the earlier `shell`/`helium`/`dolphin`/`files`/`helix`/`wine`) unless the brand is confirmed against its upstream `source` URL. A wrong match ships another company's trademark, and docs/14 §3d has no room for that. Monogram is the safe default | ✅ |
